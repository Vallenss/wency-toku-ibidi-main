...
client.ownerst = ["326227990314876929", "490597833850880037", "712282397995171871", "310779453464772608"]; // KURUCU
client.evulate = []
...
client.botroles = ["772864709086412853", "729113850653179964"] // YLER GİDİNCE YETKİSİ GİTMİYECEK BOTLARIN ID
let kanal = "784789754570997767" // LOG KANAL
...
client.on("guildUnavailable", async (guild) => {
    ...
    client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> Sunucu kullanılamaz hale geldiği için koruma amacıyla yetkileri kapadım!`)
});
...
client.on("guildBanAdd", async (guild, member) => {
    await guild.fetchAuditLogs({
        ...
        if (banLimit == 3) {
            client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi <@${hedef.id}> | (\`${hedef.id}\`) kişisini sağ tık yöntemiyle yasakladığı için sunucudan yasaklandı!`)
            let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
            guild.roles.cache.filter(a => arr.some(x => a.permissions.has(x)) == true && guild.members.cache.get(client.user.id).roles.highest.rawPosition > a.rawPosition && !client.botroles.includes(a.id)).map(huh => {
                //client.roleBackup.set(huh.id, huh.permissions.bitfield)
                huh.setPermissions(0)
...
client.on("guildUpdate", async (oldGuild, newGuild) => {
    await newGuild.fetchAuditLogs({
        ...
        if (oldGuild.name !== newGuild.name) {
            newGuild.setName("☨ Dark Paradise 🎅")
            newGuild.members.ban(yapan.id, {
                reason: "Sunucu ismi değiştirmek."
            })
            client.blackList.push(yapan.id)
            client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi tarafından sunucu ismi değiştirildi. Kişi banlandı, Sunucu ismi eski haline çevirildi.`)
        }
    ...
    await newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.name !== newChannel.name) {
            ...
            if (limitOfChannel.length == 2) {
                ...
                client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi ${limitOfChannel.length} kanalın ismini değiştirdiği için yasaklandı.Değiştirmeye çalıştığı kanal isimleri aşağıda belirtilmiştir.\`\`\`${mapped.join("\n")}\`\`\``)
...
client.on("roleUpdate", async (oldRole, newRole) => {
    await newRole.guild.fetchAuditLogs({
        ...
        if (oldRole.name !== newRole.name) {
            client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> (\`${yapan.id}\`) | <@${yapan.id}> kişisi ${oldRole.name} rolün ismini değiştirdiği için sunucudan yasaklandı.`)
...
client.on("channelCreate", async channel => {
    await channel.guild
        ...
        .then(async audit => {
            ...
            if (limit.length == 3) {
                client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi toplam 3 kanal açtığı için sunucudan yasaklandı kanallar siliniyor. Açtığı kanallar \`\`\`${limit.map(x => channel.guild.channels.cache.get(x).name).join("\n")}\`\`\``);
    ...
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            if (everyonePermission.includes("MENTION_EVERYONE") || everyonePermission.includes("MANAGE_CHANNELS")) {
                ...
                client.blackList.push(yapan.id)
                client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi ${newChannel.name} kanalının everyone izinlerine gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
...
client.on("guildBanRemove", async (guild, member) => {
    ...
    await guild.fetchAuditLogs({
        ...
        if (client.ownerst.includes(yapan.id)) return
        if (Date.now() - ayar.createdTimestamp > 5000) return;
        client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> | (\`${yapan.id}\`) kişisi daha önceden guard tarafından ban yiyen <@${member.id}> | (\`${member.id}\`) kişisinin yasağını kaldırdığı için banlandı !`)
        await guild.members.ban(yapan.id, {
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            client.blackList.push(yapan.id)
            client.channels.cache.get(kanal).send(`<:tokuchi_no:772898100603387944> <@${yapan.id}> kişisi ${newChannel.name} kanalına gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
        }
    });
});
